...
## Сессия 2026-08-11 — фикс кодировки строк (v1.6, повторный выпуск)

- **Весь не-английский интерфейс — кракозябры**: коммит 18b3890 (удаление GPS) пересохранил все 4 `strings.xml` в двойной кодировке (UTF-8 прочитан как Windows-1251 и записан снова в UTF-8; заодно добавлен BOM, поломаны «…», «°», «°C/°F», чешская диакритика). Файлы оставались валидным UTF-8 — проверка кодировки «в лоб» их не ловила, нашлось через `aapt2 dump resources` на APK из релиза
- **Фикс**: 4 файла восстановлены из c03f1ef (до поломки), удалены только 5 GPS-строк (use_gps, gps_*), BOM убран; подпись и versionCode не менялись — релиз v1.6 пересоздан на `734c9a7`, установка поверх работает
- Урок: strings.xml в этом проекте править ТОЛЬКО UTF-8-инструментами (не PowerShell с дефолтной ANSI-кодировкой)

## Сессия 2026-08-11 — перевод названий городов

- **Название добавленного города не переводилось при смене языка**: Open-Meteo перестал отдавать `local_names` в ответе геокодинга (имя приходит сразу на языке запроса), поэтому у сохранённых городов `localNames` пуст, а `CityNameResolver` фолбэчил на исходное имя
- Проверено на API: имя матчится только с языком запроса («Киев»+ru находит столицу, «Киев»+en — только одноимённую деревню в Пермском крае); латиница («Kyiv») матчится с любым языком и переводится корректно («Kyiv»→«Київ», «Kyiv»→«Kyjev»)
...
## Сессия 2026-08-26 — температура виджета не обновляется

- **Симптом**: температура в виджете менялась только после захода в приложение, причём «иногда со второго раза»; по расписанию (2/12/24 ч) не обновлялась
- **Причина 1 («со второго раза»)**: `MainActivity.onCreate` перерисовывал виджет ДО того, как `HomeViewModel.performLoad` скачивал свежие данные. Свежая погода попадала в файловый кэш (`WeatherRepository.getWeather` → `cacheWeather`), но виджет не перерисовывался; при следующем запуске onCreate читал уже обновлённый кэш — отсюда лаг «через один цикл открытия»
...
- **Главный экран — переключатель городов в TopAppBar**: ряд `AssistChip` с названиями избранных городов вынесен из контента (`PhoneLayout`/`TabletLayout`) в `Column` над `TopAppBar`, виден только при `favouriteCities.size > 1`; pager остаётся для свайпа между городами
- **`KeepAliveService` (foreground, `START_STICKY`) — тумблер «Перезапуск при закрытии»**: новый раздел «Другое» в настройках; включённый сервис с foreground-уведомлением (канал `keep_alive`, `PRIORITY_MIN`) перезапускает процесс при swipe-to-close (`onTaskRemoved` → `startForegroundService`), чтобы WorkManager продолжал обновлять виджет/уведомления по расписанию. На Android 14+ — `foregroundServiceType="specialUse"` + `PROPERTY_SPECIAL_USE_FGS_SUBTYPE`. Force-stop всё равно останавливает сервис (системное ограничение). На агрессивных прошивках (MIUI/HyperOS и т. п.) пользователю нужно вручную отключить оптимизацию батареи
- **Избранное и виджет — отдельные источники правды**: в `actions` TopAppBar при `favouriteCities.size > 1` — кнопка-бэйдж «Избранные города» (`Bookmark`+`Badge` со счётчиком), по тапу раскрывается карточка со списком избранных (текущий выделен primary/жирным). `IconToggleButton` со `Star`/`StarBorder` в поиске и для recent — единый визуальный/тактильный фидбек
- **Виджет привязан к `favouriteCities[0]`**: `WidgetUpdateManager.updateFromTargetCity(ctx)` грузит погоду для первого избранного (или текущего, если избранное пусто); `cachedForCity` запоминает принадлежность кеша. `HomeViewModel`/`WeatherUpdateWorker` сообщают `cityName` при `updateAllWidgets` — кеш обновляется только если город совпадает. После `addFavouriteCity`/`removeFavouriteCity`/`moveFavouriteCity`/`setFavouriteCities` виджет сразу подтягивает новый целевой город. Источник города в виджете отделён от текущего города на главном экране
- **Фикс `SettingsDataStore.upsert`**: `take(if (atTop) MAX_RECENT_CITIES else size)` обрезал результат по размеру **исходного** списка — `addFavouriteCity` (без `atTop`) возвращал пустой/обрезанный список, из-за чего в избранное «не добавлялось». Обрезка теперь применяется только при `atTop = true`. Звёздочка и бэйдж в TopAppBar работают как задумано
- Документация (AGENTS.md, docs/) актуализирована под актуальный код
- **Главный экран — выравнивание метрик**: `WeatherDetailItem` (осадки/влажность/УФ/ветер/давление/восход/закат в `CurrentWeatherCard`) внутри `Row(Arrangement.SpaceBetween)` использовал `Column(horizontalAlignment = CenterHorizontally)` — короткая подпись («Ветер», «Давление») уезжала под центр длинного значения, визуально не прижималась к краю ячейки. Фикс: `horizontalAlignment = Alignment.Start` — и значение, и подпись прижаты к одному (левому) краю ячейки, как в `DailyDetailItem`

## Сессия 2026-09-09 — температура виджета при старте системы и кэш (релиз 1.19)

- **Симптом**: при старте/перезагрузке системы (и периодически в текущем порядке) в виджет не подтягивалось значение погоды (показывало `--°`).
- **Причины**:
  1. В манифесте не было разрешения `RECEIVE_BOOT_COMPLETED` и `BootReceiver` — при включении телефона служба `KeepAliveService` не поднималась, а `PeriodicWorkRequest` ждал планового окна (1–2 часа).
  2. При превышении TTL (4 часа) `WeatherCache.isExpired()` вызывал `clear()`, физически удаляя файлы кэша. Виджет оставался без резервных данных.
  3. Общий файл кэша перезаписывался данными других городов при их просмотре на главном экране.
  4. В `WidgetUpdateManager.updateFromTargetCity` условие `if (cachedForCity == target.name) return` блокировало обновление при совпадении города, игнорируя устаревание данных.
  5. Виджет `ClockTempWidget` был пассивен и не инициировал загрузку при `weather == null`.
  6. У воркера отсутствовало сетевое ограничение `NetworkType.CONNECTED`.
- **Фикс**:
  1. `BootReceiver` с `RECEIVE_BOOT_COMPLETED` и `MY_PACKAGE_REPLACED`: автозапуск `KeepAliveService` (если включен тумблер) и немедленный запрос погоды (`enqueueImmediate`) при появлении сети.
  2. Из `WeatherCache` убран `clear()` при истечении TTL; добавлен параметр `allowExpired = true` (fallback при сбоях сети); выделен изолированный кэш целевого города виджета `widget_weather_cache.json`.
  3. `WidgetUpdateManager`: учет времени последнего обновления `cachedTimestamp`, сохранение в кэш виджета, автоподтягивание целевого города виджета при открытии другого города в приложении.
  4. `ClockTempWidget` и `ClockTempWidgetReceiver`: запуск `enqueueImmediate` при отсутствии данных и в `onUpdate`/`onEnabled`.
  5. `WeatherUpdateScheduler`: добавлены `Constraints(NetworkType.CONNECTED)` и метод `enqueueImmediate` с `ExistingWorkPolicy.REPLACE`.
  6. Юнит-тесты `WeatherCacheTest`.
- Релиз **v1.19** (`b182d4e`)

## Сессия 2026-09-09 — стеклянный редизайн главного экрана (релиз 1.20)

- **Что**: главный экран переведён на «матовое стекло» по макетам (аврора/стекло): новый `GlassCard` (полупрозрачный фон + градиентная фаска), `SkyPalette` (градиент неба по WMO-коду и дню/ночи, `isDayNow` сравнивает ISO-время с восходом/закатом), топбар прозрачный, список избранных на стекле, заголовок у почасового (`hourly_forecast` в 4 локалях), точка-маркер на шкале AQI
- **Адаптив**: два флага — `LocalSkyDark` (хром поверх неба: топбар, заголовки, лоадер) и `LocalGlassDark` (стекло и текст в карточках — от яркости surface темы приложения, не от неба). Тёмная тема — тонировка в тёмное + белый текст, светлая — в белое + чернильный. Гибрид «тёмное стекло на светлом небе» (день, снег/туман) — плотная заливка Black 45%, иначе белый текст не читается
- **Спарклайн температуры** в карточке почасового: Canvas, сглаженный Catmull-Rom через `cubicTo`, заливка под линией, точки. Цвет посегментно по температуре (порог 0°C из сырых данных — корректно и при °F): ниже нуля синий (к −20° густеет), выше — оранжево-красный (к +30°), переход через ноль виден сменой цвета
- Релиз **v1.20**

## Сессия 2026-09-09 — доработки стекла и почасового (релиз 1.21)

- Тёмное стекло — тонировка в чёрное (Black 45%), а не почти прозрачное
- Недельный прогноз: убран дубль температуры в шапке дневной карточки (осталась только строка «Температура» с иконкой)
- Почасовой: спарклайн температуры внутри карточки (Catmull-Rom через `cubicTo`), линия и чипы в одном скролле — точки строго под метками времени; цвет посегментно по температуре с порогом 0°C (синий/красно-оранжевый, насыщенность от удаления от нуля); учитывает интервал прогноза из настроек (1/3/6 ч)
- Релиз **v1.21**