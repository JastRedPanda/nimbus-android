...
- **ClockTempWidget** компакт: время слева, температура справа — в любом размере; порог 200→160dp; в маленьком размере мельче шрифт (16/22sp), без описания погоды
...
- **Шрифты виджета**: время 65sp, температура 68sp (пользователь: «не меньше 65sp»); компакт (<160dp): 18/22sp
...
## Вне скоупа (идеи, отложенные)

- Планшетный `NavigationRail` для настроек (screenWidth>=600) с `WindowSizeClass`
- Автоинкремент `versionCode` при сборке